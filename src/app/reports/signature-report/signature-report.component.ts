...
@Component({
  ...
  styleUrls: ['./signature-report.component.css']
})
export class SignatureReportComponent implements OnInit{
  ...
  columnDefs: ColDef[] = [                                                //change the feild name later based on API response.
    ...
    { headerName: "Account No", field: 'account_no', resizable: true, width: 130 },
    { headerName: "Bank", field: 'bank', resizable: true, width: 130 },
    { headerName: "Branch", field: 'branch', resizable: true, width: 130 },
    { headerName: "IFSC Code", field: 'ifsc_code', resizable: true, width: 130 },
  ...
  Signature() {
    if (this.searchForm.valid) {
      ...
      this._reportService.signatureReport(this.searchModel).subscribe(response => {
