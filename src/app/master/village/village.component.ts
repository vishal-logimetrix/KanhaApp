...
export class VillageComponent implements OnInit{
  ...
  originalData: any[] = [];
    ...
    rowData: any = [];
    columnDefs: ColDef[] = [
      { headerName: 'State Code', field: 'state_code', resizable: true, width:130 },
      { headerName: 'State Name', field: 'state_name', resizable: true, width:150 },
      { headerName: 'District Code', field: 'district_code', resizable: true, width:130 },
      { headerName: 'District Name', field: 'district_name', resizable: true, width:150 },
      { headerName: 'Tehsil Code', field: 'tehsil_code', resizable: true, width:130 },
      { headerName: 'Tehsil Name', field: 'tehsil_name', resizable: true, width:150 },
      { headerName: 'Village Code', field: 'village_code', resizable: true, width:130 },
      { headerName: 'Village Name', field: 'village_name', resizable: true, width:150 },
      { 
        headerName: 'Action', 
        field: 'Action',
  ...
  constructor(private _masterService: MasterService, private _serverValidation: ServerValidationService, private _msgProperty: MessageTranslationPipe,private _toastrService: ToastrService){
    ...
    this.searchForm = new FormGroup({
      ...
      tehsil_code: new FormControl('', Validators.required),
      tehsil_name: new FormControl(''),
      village_code: new FormControl(''),
  ...
  onEditClick(rowData:any): void {
    ...
    this.searchForm.patchValue({
      ...
      village_code: this.selectedRowData.village_code,
      village_name: this.selectedRowData.village_name,
    });
    ...
    this.searchForm.get('tehsil_code')?.disable();
    this.searchForm.get('tehsil_name')?.disable();
  }
  ...
  //       row.village_code.toLowerCase().indexOf(this.searchTerm) !== -1  ||
  //       row.village_name.toLowerCase().indexOf(this.searchTerm) !== -1 
  //     );
  //   }else {
  ...
  exportDataToExcel(): void {
    const formattedData = this.rowData.map((entry: any) => ({
      ...
      'VILLAGE CODE': entry.village_code,
      'VILLAGE NAME': entry.village_name,
    }));
  ...
  VillageList() {
    ...
    this.searchForm.get('tehsil_code')?.enable();
    this.searchForm.get('tehsil_name')?.enable();
    if (this.searchForm.valid) {
      this.searchModel = Object.assign({}, this.searchForm.value);
      this._masterService.VillageList(this.stateId).subscribe((res) => {
  ...
  StateDropdown() {
    this._masterService.StateDropDown().subscribe((res) => {
      this.searchResponse = res as Response;
      if (this.searchResponse.responseStatus == 200) {
  ...
  DistrictDropdown() {
    this.searchModel = Object.assign({}, this.searchForm.value);
    this._masterService.DistrictDropDown(this.stateId).subscribe((res) => {
        this.searchResponse = res as Response;
        if (this.searchResponse.responseStatus == 200) {
  ...
  TehsilDropdown() {
   ...
   this.stateId.district_code = this.searchModel.district_code;
    this._masterService.TehsilDropDown(this.stateId).subscribe((response) => {
        this.searchResponse = response as Response;
        if (this.searchResponse.responseStatus == 200) {
          this.tehsilDropDown = this.searchResponse.responseData as Dropdown[];
          this.stateId.tehsil_code = this.tehsilDropDown[0].value;
        } else {
          this._toastrService.error(this.searchResponse.responseData);
        }
      });
