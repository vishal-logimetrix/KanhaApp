...
export class FarmerCollectionListComponent implements OnInit{
  ...
  originalData: any[] = [];
    ...
    columnDefs: ColDef[] = [
      ...
      { headerName: "F Short Code", field: 'farmer_short_code', resizable: true, width: 130 },
  ...
  constructor(private _milkCollectionService: MilkCollectionService, public _masterService: MasterService, private _toastrService: ToastrService){
  ...
  exportDataToExcel(): void {
    const formattedData = this.rowData.map((entry: any) => ({
      ...
      'MILK TYPE': entry.milk_type_name,
      'SAMPLE NUMBER': entry.sample_no,
      'QTY': entry.qty,
      'FAT': entry.fat,
      'SNF': entry.snf,
      'RATE': entry.rate,
  ...
  GetFarmerCollectionList() {
    if (this.searchForm.valid) {
      ...
      this._milkCollectionService.GetFarmerCollectionList(this.newModel).subscribe(response => {
